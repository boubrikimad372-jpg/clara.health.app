...
import { Globe, ShieldAlert, Heart, ArrowRight, Loader2, Sparkles, Copy, Download, RefreshCw, CheckCircle2, ClipboardList, AlertCircle, FileText, LogIn, LogOut } from 'lucide-react';
        ...
        <AnimatePresence mode="wait">
          ...
          {screen === 'RESULTS' && analysisResult && (
            <motion.div key="results" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl w-full space-y-10">
              ...
              {!currentUser && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-[#FDF6F6] border-2 border-[#CF7E7E] rounded-2xl p-6 flex flex-col gap-4 text-center">
                  <p className="font-bold text-[#353D2D]">
                    {isRTL ? '💾 سجّلي بحساب Google لحفظ تقاريرك' : '💾 Sign in with Google to save your reports'}
                  </p>
                  <button onClick={handleGoogleSignIn} disabled={isSigningIn} className="flex items-center justify-center gap-3 bg-white border-2 border-[#CF7E7E] text-[#353D2D] py-4 rounded-2xl font-black hover:bg-[#FDF6F6] transition-all disabled:opacity-50">
              ...
              {currentUser && (
                <div className="flex items-center justify-between bg-[#F4F7F2] rounded-2xl p-4">
                  <p className="text-sm font-bold text-[#617250]">✓ {currentUser.displayName || currentUser.email}</p>
                  <button onClick={signOutUser} className="flex items-center gap-2 text-xs text-[#617250] hover:text-[#353D2D]">
