...
export default function App() {
  ...
  const handleNextStep = async () => {
    ...
    // FIX: build updatedAnswers here and pass directly — don't rely on setState being synchronous
    ...
    if (currentStepIndex < analysisResult.steps.length - 1) {
      ...
      setCurrentAnswer('');
      // Store finalAnswers in ref so handleFinalAnalysis reads the latest value
  ...
  const handleFinalAnalysis = async () => {
    ...
    // FIX 3: use ref to guarantee latest answers (setState is async)
    const latestAnswers = finalAnswersRef.current && Object.keys(finalAnswersRef.current).length > 0
    ...
    const finalUserData = {
      ...
      age: userData.age,
      interviewAnswers: latestAnswers,
      seenDoctorBefore: userData.seenDoctorBefore,
      doctorFindings: userData.doctorFindings,
    };

    try {
      const finalResult = await analyzeStructured(finalUserData, uiLanguage, outputLanguage);
      setAnalysisResult(finalResult);
      // FIX 1: reset streamedNarrative and track streaming with a local flag
      ...
      streamNarrative(
        ...
        chunk => {
          ...
          setStreamedNarrative(accumulatedText);
        },
        fullText => {
          // FIX 1: always set the narrative from fullText, never leave placeholder
          setAnalysisResult(prev =>
            prev ? { ...prev, clinicalReport: { ...prev.clinicalReport, narrative: fullText || accumulatedText } } : prev
          );
          setStreamedNarrative('');
        ...
        err => {
          ...
          // FIX 1: if streaming fails but we accumulated text, use it
          if (accumulatedText.trim()) {
            setAnalysisResult(prev =>
              prev ? { ...prev, clinicalReport: { ...prev.clinicalReport, narrative: accumulatedText } } : prev
            );
            setStreamedNarrative('');
          } else {
                ...
                isOutputLangStep ? (
                  ...
                  <div className="bg-white rounded-[2.5rem] p-10 shadow-2xl border border-[#F3F4F6]">
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                          {analysisResult.steps[currentStepIndex].suggestions.map((s, i) => {
                            // Split "1-3 (Mild)" into number part and label part for cleaner display
                              ...
                              <button key={i} onClick={() => setCurrentAnswer(s)}
                                ...
                                {match ? (
                                  <span className="flex flex-col gap-0.5">
                                    <span className="font-black text-base">{match[1]}</span>
                                    <span className="text-xs opacity-80">{match[2]}</span>
                                  </span>
                                ) : s}
                              </button>
                            );
                    ...
                    <div className="flex justify-between items-center mt-10">
                      <button onClick={handleBack} className="px-8 py-4 bg-white border-2 border-[#E9EFE5] rounded-2xl font-bold hover:bg-[#F3F4F6]">{t.analysis.back}</button>
                      {/* FIX 2: last question is optional — always allow next */}
                      <button
                        onClick={handleNextStep}
                        disabled={currentStepIndex < analysisResult.steps.length - 1 && !currentAnswer.trim()}
                        className={cn("px-12 py-5 rounded-2xl font-bold flex items-center gap-3",
                          (currentStepIndex === analysisResult.steps.length - 1 || currentAnswer.trim())
          ...
          {screen === 'RESULTS' && analysisResult && (
            <motion.div key="results" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl w-full space-y-8">
                ...
                <div className="mb-10">
                  ...
                  <div className="border-2 border-black p-6 text-lg font-bold leading-relaxed bg-[#F9FAF9] min-h-[100px]" style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                    {isStreaming ? (
                      ...
                      // FIX 1: prefer the stored narrative; only fall back to streamedNarrative if narrative is empty or placeholder
                      (() => {
                        const stored = analysisResult.clinicalReport.narrative;
                        const isPlaceholder = !stored || stored.trim().length < 30 || stored.toLowerCase().includes('will describe');
                        return isPlaceholder ? streamedNarrative || stored : stored;
                      })()
                    )}
                  </div>
                </div>
              ...
              <div className="space-y-4">
                ...
                {/* Browser translate warning */}
                <div className="bg-amber-50 border-2 border-amber-300 rounded-2xl p-4 flex items-start gap-3">
                  <AlertCircle className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" />
                  <p className="text-sm font-bold text-amber-800">{t.results.translateWarning}</p>
                </div>
