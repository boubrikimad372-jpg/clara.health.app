...
export default function App() {
  ...
  const [screen, setScreen] = useState<'DISCLAIMER' | 'WELCOME' | 'INTAKE' | 'DEMOGRAPHICS' | 'ANALYSIS' | 'RESULTS'>('DISCLAIMER');
  ...
  const handleBack = () => {
    ...
    else if (screen === 'INTAKE') setScreen('DEMOGRAPHICS');
      ...
      try {
        ...
        if (hasForbiddenTerms) {
          ...
          finalResult = await analyzeExperience({
            ...
            interviewAnswers: updatedAnswers,
            seenDoctorBefore: false,
            doctorFindings: "REWRITE MANDATORY: Use 'I' voice only. No third-person."
          }, language);
        }
            ...
            <motion.div key="demographics" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="max-w-md w-full bg-white rounded-[2.5rem] p-10 shadow-xl border border-[#F3F4F6] text-center">
              ...
              <div className="space-y-6">
                ...
                <div className="flex gap-4">
                  ...
                  <button onClick={() => setScreen('INTAKE')} disabled={!userData.age} className={cn("flex-[2] py-5 rounded-2xl font-bold transition-all flex items-center justify-center gap-3 shadow-lg", userData.age ? "bg-[#CF7E7E] text-white hover:bg-[#B85F5F] shadow-[#F7DADA]" : "bg-[#E9EFE5] text-[#B8CCAA] cursor-not-allowed")}>
          ...
          {screen === 'RESULTS' && analysisResult && (
            <motion.div key="results" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl w-full space-y-10">
              ...
              <div className="space-y-6">
                ...
                <button onClick={() => { setScreen('WELCOME'); setAnalysisResult(null); setUserData({ intakeText: '', age: '', interviewAnswers: {} }); }} className="w-full py-5 bg-gray-200 text-black rounded-2xl font-black uppercase tracking-wider hover:bg-gray-300 transition-all flex items-center justify-center gap-3">
